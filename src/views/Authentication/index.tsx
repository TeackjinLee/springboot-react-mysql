    ...
    const SignUpCard = () => {
        ...
        const signUpResponse = (responseBody: SignUpResponseDto | ResponseDto | null) => {
            ...
            if (code === 'DT') {
                ...
                setTelNumberErrorMessage('중복되는 핸드폰 주소입니다.');
            }
            if (code === 'VF')  alert('모든 값을 입결하세요.');
            if (code === 'DBE') alert('데이터베이스 오류입니다.');
        ...
        const onNicknameChaneHandler = (event: ChangeEvent<HTMLInputElement>) => {
            const {value} = event.target;
            setNickname(value);
        ...
        const onTelNumberChaneHandler = (event: ChangeEvent<HTMLInputElement>) => {
            const {value} = event.target;
            setTelNumber(value);
        ...
        const onAddressChaneHandler = (event: ChangeEvent<HTMLInputElement>) => {
            const {value} = event.target;
            setAddress(value);
        ...
        const onAddressDetailChaneHandler = (event: ChangeEvent<HTMLInputElement>) => {
            const {value} = event.target;
        ...
        const onNextButtonClickHandler =  () => {
            const emailPatter = /^[a-zA-Z0-9]*@([-.]?[a-zA-Z0-9])*\.[a-zA-Z]{2,4}$/;
            const isEmailPattern = emailPatter.test(email);
            if (!isEmailPattern) {
                setEmailError(true);
        ...
        const onSignUpButtonClickHandler = () => {
            ...
            if (!hasNickname) {
                setNicknameError(false);
                setNicknameErrorMessage('닉네임을 입결해주세요.');
            }
            ...
            if (window.confirm('이 정보로 회원가입 하시겠습니까?')) {
            ...
            <div className='auth-card'>
                <div className="auth-card-box">
                    <div className="auth-card-top">
                        ...
                        {page === 2 && (
                            <>
                                <InputBox ref={nicknameRef} label='넥네임*' type='text' placeholder='닉네임을 입력해주세요.' value={nickname} onChange={onNicknameChaneHandler} error={isNicknameError} message={nicknameErrorMessage} onKeyDown={onNicknameKeyDownHandler}/>
                                <InputBox ref={telNumberRef} label='휴대폰번호*' type='text' placeholder='핸드폰 번호를 입력해주세요.' value={telNumber} onChange={onTelNumberChaneHandler} error={isTelNumberError} message={telNumberErrorMessage} onKeyDown={onTelNumberKeyDownHandler}/>
                                <InputBox ref={addressRef} label='주소*' type='text' placeholder='우편번호 찾기' value={address} onChange={onAddressChaneHandler} error={isAddressError} message={addressErrorMessage} icon='expand-right-light-icon' onButtonClick={onAddressButtonClickHandler} onKeyDown={onAddressKeyDownHandler}/>
                                <InputBox ref={addressDetailRef} label='상세 주소' type='text' placeholder='상세주소를 입력해주세요.' value={addressDetail} onChange={onAddressDetailChaneHandler} error={false} onKeyDown={onAddressDetailKeyDownHandler}/>
                            </>
                        )}
                    </div>
                    <div className="div-card-bottom">
                        {page === 1 && (
    ...
    return (
        <div id='auth-wrapper'>
            <div className="auth-container">
                <div className="auth-junbotron-box">
                    <div className="auth-jumbotron-contents">
                        <div className="auth-log-icon"></div>
